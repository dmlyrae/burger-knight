...
export const SingleOrder: React.FunctionComponent<IOrderProps> = ({ order, status }) => {
	...
	return (
			...
			>
				...
				<p className={cl.number}>#{order.number}</p>
				<p className={cl.date}>{getTimeFromTimestamp(order.createdAt)}</p>
				</div>
				<h3 className={cl.title} style={status ? { marginBottom: 0 } : { marginBottom: 24 }}>{order.name}</h3>
				{
				status && (
				...
				<ul className={cl.list}>
					<li key={uuidv4()} className={cl.ingredient} style={{ zIndex: 6 }}>
				...
				<div className={cl.sum}><p className={cl.price}>{price}<span className={cl.icon}><CurrencyIcon type="primary" /></span></p></div>
				</div>
			</article>
	)
}